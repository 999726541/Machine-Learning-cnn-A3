...
def NNUpdate(model, eps, momentum):
    ...
    model['b3_V'] = momentum * model['b3_V'] + eps * model['dE_db3']
...
def main():
    ...
    DisplayPlot(stats['train_ce'], stats['valid_ce'], 'Cross Entropy', number=0, time= 1000)
    DisplayPlot(stats['train_acc'], stats['valid_acc'], 'Accuracy', number=1, time=1000)
